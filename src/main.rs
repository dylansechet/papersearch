...
fn main() -> Result<()> {
    ...
    info!("Reading paper references...");
    let references: Vec<String> = BufReader::new(fs::File::open("doi_list.txt")?)
        .lines()
    ...
    let graph = fetch_citation_graph(references, Config::default())?;

    let stats = graph.stats();

    info!("Writing to citations.json...");
    fs::write("citations.json", serde_json::to_string_pretty(&graph)?)?;
